...
class SmallDgrEncoder(nn.Module):
...
# class MediumDgrEncoder(nn.Module):
#
#     def __init__(self, dropout):
#         super().__init__()
#         conv1 = mod.ConvBlock(c_in=3, c_out=36, kernel_size=4, stride=1, padding=0)
...
class DgrInstanceSpaceNNSmall(models.InstanceSpaceNN):
    ...
    def __init__(self, device):
        ...
        encoder = SmallDgrEncoder(dropout)
...
class DgrResNet18(models.MultipleInstanceNN):
    ...
    def __init__(self, device):
        super().__init__(device, DgrLucSingleInstanceDataset.n_classes, DgrLucSingleInstanceDataset.n_expected_dims)
        self.device = device
