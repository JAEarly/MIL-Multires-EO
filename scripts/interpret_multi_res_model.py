...
from floodnet.floodnet_interpretability_multi_res import FloodNetMultiResInterpretabilityStudy
...
def run():
    ...
    study = FloodNetMultiResInterpretabilityStudy(device, test_dataset, model, args.show_outputs)

    if args.task == 'sample':
        study.sample_interpretations()
    elif args.task == 'specific':
        study.create_interpretation_from_id(6279)
    else:
